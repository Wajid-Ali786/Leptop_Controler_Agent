...
**Implementation notes — emergency-stop global hotkey (recorded 18 September 2026).** The hotkey is `Ctrl+Alt+Backspace` and **stays as it is**: it was tested physically and meets the requirement. The response-time threshold is deliberately **not set and not encoded anywhere yet**.
...
- **Established observations — physical keyboard, this machine, 18 September 2026.** These are measurements, not inferences:
  ...
  4. Synthetic presses fired `WM_HOTKEY` **65 times out of 65** across the idle, typing, scrolling and one-shot measurement trials.
  5. **The earlier "NOT RECEIVED" runs with Chrome or the Claude app in front were timing-related — the key was not pressed inside the 60-second window — and are *not* evidence that Chrome blocks the hotkey.** Recorded explicitly so this wrong conclusion is not inherited later.
...
- **Elevated windows: UNKNOWN.** Whether the hotkey still reaches the listener while a window running as administrator is in front has **not been measured on this machine**. It is not assumed to work and not assumed to fail. Nothing in the code or the docs depends on an answer. Windows' secure desktop (the UAC prompt itself, Ctrl+Alt+Delete) is out of scope by design: no ordinary program receives input there.
...
- **Measured response time (synthetic input only).** 65 trials. Press → action stopped: **22.4 ms worst case** overall (idle 1.2 ms max; typing 5.6 ms max; scrolling 22.4 ms max; one-shot 3.2 ms max). No input continued after a stop, and no stop was ever offered as retryable. Timings use `time.perf_counter()`, because `time.monotonic()` only moves in ~15.6 ms steps on Windows — coarser than the thing being measured. The press-to-`WM_HOTKEY` and press-to-stopped figures are **synthetic**: they exclude the physical keyboard path, which is covered by observations 2 and 3 above but has not been timed.
